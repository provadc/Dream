...
echo file_get_contents("usate2.txt");			 
include "../../../config.php"; 
$pagina=1;
if(isset($_GET['page'])){
    $pagina=$_GET["page"];}
$res="select count(Num_Telaio)as conta FROM Auto WHERE Nuova=1 AND Num_Telaio NOT IN(select Num_Telaio from StoricoVendite)";
$result =mysqli_query($conn, $res);
$row=$result->fetch_assoc();
$conta=$row["conta"]; 
$sql = "SELECT Num_Telaio, marca, modello, prezzo, Immagine FROM Auto WHERE Nuova=1 AND Num_Telaio NOT IN(select Num_Telaio from StoricoVendite) LIMIT 10 OFFSET ". ($pagina-1)*10;
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<h2 id='km'>".$row["km"]." km"."</h2>";
        echo "<h3 id='prezzo'>". $row["prezzo"]." ".€."</h3>"."</div>"."</a>";
    }    
    echo"<div id='navigatori'>";
    if($pagina>1){
        echo "<div id='precedente' ><a href='usate.php?page=". ($pagina-1) ."'><div>Precedente</div></a></div> ";   
    }
    else{
    ...
    if($conta>10*($pagina)){
        echo "<div id='successivo' > <a href='usate.php?page=". ($pagina+1) ."'><div>Successiva</a></div>";}
    else{
